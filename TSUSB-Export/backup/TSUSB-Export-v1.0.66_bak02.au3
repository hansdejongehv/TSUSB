...
Global $TStimelogfile="TurtleStTimelog.txt"
...
Global Const $retFailed = 0
Global Const $retContinueEject = 1
Global Const $retContinueNoEject = 2
Global Const $retExported = 3
...
Global $msgNoDST, $msgNoXML, $msgInvalidName, $msgTooMuchTimeDiff, $msgTooManyColors, $msgTooLarge, $msgTooManyStitches, $msgNoStitches, $msgCopyRetry, $msgUpdRetry, $msgNoUSBStick, $msgCopyError
...
Global $msgXMLDSTnamesNotMatching, $msgCloudSaving, $mlblPassword,$mlblST, $NewestDSTFileTime, $NewestXMLFileTime, $XML2DSTdiff, $NewestXMLFilePath, $hdrDetails, $hdrDone, $msgForbiddenLabel
...
Global Const $msgForbiddenNameFlag = 1
Global Const $msgXMLDSTnamesNotMatchingFlag = 2
Global Const $msgTooMuchTimeDiffFlag = 4
Global Const $msgTooManyColorsFlag = 8
Global Const $msgTooLargeFlag = 16
Global Const $msgTooManyStitchesFlag = 32
Global Const $msgNoStitchesFlag = 64
Global Const $msgCloudSavingFlag = 128
Global Const $msgXTooLargeFlag = 256
Global Const $msgYTooLargeFlag = 512
Global Const $msgForbiddenLabelFlag=1024
...
Func SetMessagesInLanguage()
	...
	$msgNoStitches = "There are no stitches in your design. Make sure the embroidery pattern is visible on the podium before you export to .DST."
	...
	Switch $Language
		Case $LangDutch
			...
			$msgNoStitches = "Er zijn geen steken in je ontwerp. Zorg dat het patroon zichtbaar is op het podium voordat je exporteert naar DST file."
...
Func SetDynamicStrings($Flags)
	...
	If BitAND($Flags, $msgNoStitchesFlag) <> 0 Then $ErrorString = $ErrorString & $msgNoStitches & @CRLF
	...
	Else
		...
		For $i = 1 To $aArray[0]
			...
			If StringInStr($USBvolume, $USBstickPrefixTurtleSt, 1) = 1 Then
				...
				; $result can have one of these values
				; $retFailed			The copy failed for some reason. Display a message and eject the stick
				; $retContinueEject		The copy was aborted. No message to be displayed. Eject the stick
				; $retContinueNoEject	The copy was not done because e.g. user wants to continue to use the design on the stick. Do not eject the stick
				; $retExported			The copy was successful. A message has already been displayed and the stick was already ejected

				if $Result = $retContinueEject Then
					consolewrite ("1063 Driveletter=" &  $Driveletter    & @crlf )
					_WinAPI_EjectMedia($DriveLetter)
				endif
				If $result = $retFailed Then
					;MsgBox($MB_ICONINFORMATION,$hdrRetry,$msgCopyRetry)
					;If there was an error, display what to do, but first eject the stick
					_WinAPI_EjectMedia($DriveLetter)
					DisplayPanel(1, "", $hdrRetry, $msgCopyRetry, $mbtnOK, "", "", 0)
				EndIf
				consolewrite ("1061 $result="& $result & @CRLF)
...
Func CopyDesignToUSB($Folder, $USBDriveLetter)
	...
	If $result = 3 Or $result = 0 Then Return $retContinueEject
	...
	; If there are TS related files on the stick, move them to a backup folder on the stick
	If $result = 1 Or FileExists($USBDriveLetter & "\*.DST") or FileExists($USBDriveLetter & "\" & $ExtraDataFileName) or FileExists($USBDriveLetter & "\" & $TStimelogfile) Then
		local $Now= _NowCalc()
		...
		if $result = 1 then	FileMove ($USBDriveLetter & "\" & $TStimelogfile,$BackupDir)
	...
	If ($StitchCount =0 ) Then
		$ErrorFlags = $ErrorFlags + $msgNoStitchesFlag
	EndIf
	...
	If $ErrorFlags <> 0 Then
		...
		if $result<>1 then Return $retFailed
	EndIf
	...
	;	if @error=1 then Return $retFailed
	;wend
...
;~ 	EndIf
	...
	FileWriteLine($s_Path_Downloads & "\" & $TStimelogfile,"TSUSB-Export=" & $Now & @crlf)
	FileCopy($s_Path_Downloads & "\" & $TStimelogfile, $USBDriveLetter & "\" & $TStimelogfile)
...
Func DisplayPanel($Type, $Window2Disable, ByRef $HeaderText, ByRef $MessageOrFlags, ByRef $Button1, ByRef $Button2, ByRef $Button3, $ExitButtonNr = 0, $TimeOut = 0)
	...
	While $LanguageChanged = True
		...
		if $Type=1 then
			...
			if bitor (BitAND($MessageOrFlags,$msgTooManyStitchesFlag), BitAND($MessageOrFlags,$msgNoStitchesFlag))  <> 0 then
				GUICtrlSetBkColor(-1, 0xFF0000)
				GUICtrlSetColor(-1, 0xFFFFFF)
			EndIf
